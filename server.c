...
if (path[len-1]=='/')
  len--;
char *result=malloc(len+12);

//merge path with index.php and try to open a file with that location/name
memcpy(result,path,len);
memcpy(&result[len],"/index.php\0",11);
 //   memcpy(&result[len-1],"/index.php\0",11);
FILE* file1=fopen(result,"r");
if (file1!=NULL)
    {
    fclose(file1);   
    return result;
...
//merge path with index.html and try to open a file with that location/name
memcpy(result,path,len);
//if (path[len]!='/')
    memcpy(&result[len],"/index.html\0",12);
//else
//    memcpy(&result[len-1],"/index.html\0",12);
FILE* file2=fopen(result,"r");
if (file2!=NULL)
     {
    fclose(file2);   
    return result;
    }
//no such files open returns null
return NULL;
fclose(file1);
fclose(file2);
}
...
//learn how many bytes there are and store it in size and at *length
while (fread(&unit,1,1,file)!=0)
    size++;
fseek(file,0,SEEK_SET);
//printf("%i",size);
char *loaded=malloc(size+1);
fread(loaded,1,size,file);
*content=&loaded[0];
*length=size;
return true;
}
...
const char* lookup(const char* path)
{
// allocate memory for a 4 character string and then copy the last 5 chars in path,including the \0
int dotplace=0;
int len=strlen(path);
//find dotplace for 2 (.js) 3(.jpg) or 4 (.html) letters after the dot
if (path[len-5]=='.')
    dotplace=6;
if (path[len-4]=='.')
    dotplace=5;
if (path[len-3]=='.')
    dotplace=4;
//if there is no character before the dot it will still be invalid and show as null
if (len- dotplace <0) dotplace=0;
char* last4=malloc(dotplace);
 
for (int i=0;i<dotplace;i++)
    last4[i]=tolower(path[strlen(path)+i-dotplace+1]);
const char *ends[]= {".css",".html",".gif",".ico",".jpg",".js",".php",".png"};
const char *returns[] = {"text/css","text/html","image/gif","image/x-icon","image/jpeg","text/javascript","text/x-php","image/png"};
for (int k=0;k<8;k++)
    if (strcmp(ends[k],last4)==0)
        return returns[k];
free(last4);
return NULL;
}
...
int quelen=0;
int spaces=0;
bool qexists= false; //short for queryexists
...
//chek if there is a " in the-request target (also store the last char number for request and query )
int i=4;
while(line[i]!=' ')
    {
    if (line[i]=='"')
...
if (strncmp(&line[len-10],"HTTP/1.1",8)!=0)
    {
    error(505);
    return false;
    }
//copy abs_path and query
else
    {
    for (int j=4;j<=reqlen;j++)    
        abs_path[j-4]=line[j];
    abs_path[reqlen-3]='\0';
    //we ignore the q in the querry 
    if (quelen-reqlen>0)
        for (int k=reqlen+1;k<=quelen;k++)    
            {
            query[k-reqlen-1]=line[k+1];
            }
    query[quelen-1]='\0';
    }
//printf("%i %i",reqlen,quelen);
return true;
}
